...
foreach ($temperas as $key => $value) { ${"$key"} = $value; } 
 ...
 $boxes = get_theme_mod('front_boxes');
?>
        <section id="smamo-front-boxes">
            <div class="inner">
                <?php foreach($boxes as $box) : ?>
                <div class="box">
                    <div class="inner">
                        <header class="entry-header">
                            <?php if ($box['url'] !== '') { echo '<a href="'.esc_url($box["url"]).'">'; } ?>
                            <h3><?php echo esc_attr($box['title']); ?></h3>
                            <?php if ($box['url'] !== '') { echo '</a>'; } ?>
                        </header>
                        <div class="entry-image" style="background-image:url(<?php echo $box['image']; ?>);"></div>
		...
		<section id="container" class="one-column <?php //echo tempera_get_layout_class(); ?>">
			...
			if ( have_posts() ) :
				...
				$the_query = new WP_Query( array('post_type' => array('post','sync_post'), 'posts_per_page'=>$temperas['tempera_frontpostscount'],'paged'=> $paged) ); 
